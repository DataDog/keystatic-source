...
var index = require('./index-bf07e3be.node.react-server.cjs.js');
...
function validateArrayLength(schema, val, path) {
  ...
  if (((_schema$validation = schema.validation) === null || _schema$validation === void 0 || (_schema$validation = _schema$validation.length) === null || _schema$validation === void 0 ? void 0 : _schema$validation.min) !== undefined && val.length < schema.validation.length.min) {
    return new PropValidationError(new index.FieldDataError(`Must have at least ${schema.validation.length.min} element${schema.validation.length.min === 1 ? '' : 's'}`), path, schema);
  }
  if (((_schema$validation2 = schema.validation) === null || _schema$validation2 === void 0 || (_schema$validation2 = _schema$validation2.length) === null || _schema$validation2 === void 0 ? void 0 : _schema$validation2.max) !== undefined && val.length > schema.validation.length.max) {
    return new PropValidationError(new index.FieldDataError(`Must have at most ${schema.validation.length.max} element${schema.validation.length.max === 1 ? '' : 's'}}`), path, schema);
  }
}
...
validateArrayFieldLength) {
  ...
  if (schema.kind === 'conditional') {
    if (value === undefined) {
      return index.getInitialPropsValue(schema);
    }
    try {
      if (typeof value !== 'object' || value === null || isArray(value)) {
        throw new index.FieldDataError('Must be an object');
      }
      for (const key of Object.keys(value)) {
        if (key !== 'discriminant' && key !== 'value') {
          throw new index.FieldDataError(`Must only contain keys "discriminant" and "value", not "${key}"`);
        }
      }
    } catch (err) {
      throw new PropValidationError(err, path, schema);
    ...
    try {
      ...
      for (const key of Object.keys(value)) {
        if (!allowedKeysSet.has(key)) {
          throw new index.FieldDataError(`Key on object value "${key}" is not allowed`);
        }
      }
    } catch (err) {
      throw new PropValidationError(err, path, schema);
  ...
  if (schema.kind === 'array') {
    ...
    try {
      if (!isArray(value)) {
        throw new index.FieldDataError('Must be an array');
      }
    } catch (err) {
      throw new PropValidationError(err, path, schema);
    }
...
function formatFormDataError(error) {
  ...
  return flatErrors.map(error => {
    if (error instanceof PropValidationError) {
      ...
      return `${path}: ${error.cause instanceof index.FieldDataError ? error.cause.message : `Unexpected error: ${error.cause}`}`;
    }
...
function loadDataFile(data, formatInfo) {
  const parse = formatInfo.data === 'json' ? JSON.parse : jsYaml.load;
  ...
  emery.assert(res !== null, 'frontmatter not found');
  return {
