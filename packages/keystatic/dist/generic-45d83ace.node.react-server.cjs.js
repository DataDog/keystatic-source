...
var requiredFiles = require('./required-files-c0e1bb15.node.react-server.cjs.js');
...
const cache = getCache(React__namespace);
...
const readItem = cache(async function readItem(rootSchema, formatInfo, itemDir, resolveLinkedFiles, debugReference, fsReader, ...slugInfo) {
  ...
  let validated;
  try {
    validated = requiredFiles.parseProps(rootSchema, loaded, [], [], (schema, value, path, pathWithArrayFieldSlugs) => {
    ...
    const formatted = requiredFiles.formatFormDataError(err);
